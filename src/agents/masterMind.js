...
export async function fetchRealPools() {
  try {
    const res = await fetch(`${API_BASE}/api/botchain/pools?limit=5&sort=apr24h`);
...
export async function fetchDEXOverview() {
  try {
    const res = await fetch(`${API_BASE}/api/botchain/overview`);
    const json = await res.json();
...
export async function interpretGoal(userGoal) {
  const response = await groq.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `You are AgentX AI, an autonomous AI financial agent on BotChain.
...
- dexCommand: which BotChain DEX action to take
...
Respond ONLY with valid JSON, no markdown.`
  ...
  try {
    ...
    return {
      ...
      recommendedAction: 'Analyze top yield pools on BotChain DEX',
      dexCommand: 'botchain-dex pools list',
      explanation: text
    };
  }
}
...
export async function getMarketInsights(poolData) {
  const response = await groq.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `You are DataAgent, a market analysis AI on BotChain. 
Analyze the provided BotChain DEX pool data and give brief actionable insights.
Respond in 2-3 sentences max. Be direct and specific about best opportunity.`
      },
      {
        role: 'user',
        content: `Analyze these BotChain DEX pools and recommend the best yield opportunity: ${JSON.stringify(poolData)}`
      }
    ],
    temperature: 0.2,
    max_tokens: 200,
...
export async function assessRisk(strategy, riskLevel) {
  const response = await groq.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `You are RiskAgent on BotChain. Assess the risk of a DeFi strategy.
...
export async function runMasterMind(userGoal, callbacks) {
  ...
  try {
    ...
    onUpdate({ step: dataName, action: 'Hired on-chain — Scanning BotChain DEX pools...', status: 'thinking', tx: hireTx });
    await notify(telegramChatId, `📊 <b>${dataName}</b> hired on-chain\nTx: <code>${hireTx.slice(0, 20)}...</code>\n\nScanning BotChain DEX pools...`);
  } catch (err) {
    console.log('DataAgent hire failed:', err.message?.slice(0, 50));
    onUpdate({ step: dataName, action: 'Scanning BotChain DEX pools...', status: 'thinking', tx: null });
  }

  // ===== STEP 3 — DataAgent scans Byreal pools =====
  const realPools = await fetchRealPools();
  const poolsToAnalyze = realPools || [
    { pair: 'WBOT/USDT', apr: '12.86%', tvl: '$1.15M' },
    { pair: 'BOT/USDT', apr: '33.94%', tvl: '$79.30K' },
    { pair: 'BOT/USDC', apr: '426.73%', tvl: '$336.67' },
  ];
  ...
  try {
    ...
    if (strategy.intent === 'yield' && bestPool?.poolId) {
      const lpRes = await fetch(`${API_BASE}/api/botchain/lp/preview`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const perpsRes = await fetch(`${API_BASE}/api/botchain/dex/scan?risk=conservative`);
  ...
  try {
    ...
    txHashes.push(recordTx);
    onUpdate({ step: tradeName, action: `Strategy executed${tradeResult ? ' + BotChain DEX' : ''}`, status: 'done', tx: recordTx });
    await notify(telegramChatId,
      `⚡ <b>${tradeName}</b> — Strategy Executed!\n\n✅ ${strategy.recommendedAction}\n${tradeResult ? '🔗 BotChain DEX integration active' : ''}\nTx: <code>${recordTx.slice(0, 20)}...</code>`
    );
  } catch {
