...
const RPC_URL = import.meta.env.VITE_RPC_URL || 'https://rpc.bohr.life';
  ...
  useEffect(() => {
    ...
    async function fetchEvents() {
      try {
        ...
        const vault = new ethers.Contract(ADDRESSES.AgentXVault, VaultABI, provider);
        ...
        const allEvents = [
          ...
          ...hired.map(e => ({
            ...
            hash: e.transactionHash,
            amount: `${ethers.formatEther(e.args.payment || 0)} BOT`,
            status: 'Confirmed',
            blockNumber: e.blockNumber,
            timestamp: null,
          })),
          ...paid.map(e => ({
            ...
            hash: e.transactionHash,
            amount: `${ethers.formatEther(e.args.amount || 0)} BOT`,
            status: 'Confirmed',
            blockNumber: e.blockNumber,
            timestamp: null,
          })),
  ...
  useEffect(() => {
    ...
    try {
      ...
      registry.on('AgentPaid', (agentId, owner, amount, event) => {
        onNewEvent({ type: 'PAY', desc: `Agent #${agentId} paid ${ethers.formatEther(amount)} BOT`, hash: event.log.transactionHash, time: 'just now' });
      });
