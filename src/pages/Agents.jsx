    ...
    <div className="card fade-up">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '10px', marginTop: '14px', paddingTop: '14px', borderTop: '0.5px solid rgba(255,255,255,0.06)' }}>
        {[
          ...
          { label: 'BOT Earned', val: (Number(agent.totalEarned) / 1e18).toFixed(4) },
      ...
      {txHash && (
        <div style={{ marginTop: '8px', padding: '8px', background: 'rgba(255,255,255,0.03)', borderRadius: '6px', borderLeft: '2px solid #fff' }}>
          ...
          <a
            href={`${import.meta.env.VITE_EXPLORER_URL || 'https://testnet-scan.botchain.ai'}/tx/${txHash}`}
            target="_blank"
            rel="noreferrer"
    ...
    <div className="page-wrap">
      ...
      {showDeploy && (
        <div className="card fade-up" style={{ marginBottom: '20px', border: '0.5px solid rgba(255,255,255,0.2)' }}>
          ...
          <p style={{ fontSize: '12px', color: '#555', marginBottom: '16px' }}>
            Deploys ERC-8004 identity NFT on BotChain Testnet
          </p>
          ...
          {txHash && (
            <div style={{ padding: '10px', background: 'rgba(255,255,255,0.03)', borderRadius: '8px', borderLeft: '2px solid #fff' }}>
              <div style={{ fontSize: '11px', color: '#888', marginBottom: '4px' }}>Agent deployed on BotChain!</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '12px', marginBottom: '20px' }}>
        {[
          ...
          { label: 'BOT Paid Out', val: liveStats.totalEarned.toFixed(4), sub: 'total agent fees' },
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:'8px' }}>
            {leaderboard.map((a, i) => (
              <div key={i} style={{ textAlign:'center', padding:'10px 8px', background:'rgba(255,255,255,0.02)', borderRadius:'8px', border:'0.5px solid rgba(255,255,255,0.06)' }}>
                ...
                <div style={{ fontSize:'9px', color:'#444', marginTop:'2px' }}>Net BOT</div>
              </div>
            ))}
          </div>
        </div>
