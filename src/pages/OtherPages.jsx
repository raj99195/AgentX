...
const EXPLORER_URL = import.meta.env.VITE_EXPLORER_URL || 'https://testnet-scan.botchain.ai';


const TX_FILTERS = ['All', 'DEPLOY', 'HIRE', 'PAY', 'FIRE', 'DECISION'];
const EXPLORER = 'https://testnet-scan.botchain.ai/tx/';
...
export function Transactions() {
  ...
  const exportCSV = () => {
    ...
    a.href = url; a.download = 'agentx-history.csv'; a.click();
  };
    ...
    <div className="page-wrap">
      ...
      <div className="card">
        ...
        <div style={{ textAlign:'center', marginTop:'16px', paddingTop:'16px', borderTop:'0.5px solid rgba(255,255,255,0.06)' }}>
          <a href={'https://testnet-scan.botchain.ai/address/' + ADDRESSES.AgentRegistry}
            target="_blank" rel="noreferrer" className="btn btn-outline" style={{ fontSize:'12px' }}>
            View all on BotChain Explorer ↗
          </a>
        </div>
      </div>
    </div>
...
export function Settings() {
  const [t, setT] = useState(() => {
    try {
      const saved = localStorage.getItem('agentx_settings');
  ...
  useEffect(() => {
    localStorage.setItem('agentx_settings', JSON.stringify(t));
  }, [t]);
  ...
  const testTelegram = async () => {
    ...
    try {
      ...
      setTgMessage('BotChain DEX /status command sent!');
    ...
    <div className="page-wrap" style={{ maxWidth:'800px' }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
        ...
        <div className="card" style={{ border: tgStatus === 'success' ? '0.5px solid rgba(255,255,255,0.2)' : '0.5px solid rgba(255,255,255,0.06)' }}>
          <div className="card-label">Telegram + BotChain DEX</div>
          <p style={{ fontSize:'12px', color:'#555', marginBottom:'14px' }}>
            Connect Telegram to receive agent notifications and control BotChain DEX
          </p>
          ...
          <div style={{ padding:'10px', background:'rgba(255,255,255,0.02)', borderRadius:'8px', marginBottom:'12px' }}>
            ...
            <p style={{ fontSize:'11px', color:'#555' }}>1. Open <a href="https://t.me/mantlemind_bot" target="_blank" rel="noreferrer" style={{ color:'#888' }}>@agentx_bot</a> on Telegram</p>
        ...
        {/* BotChain DEX Integration */}
        <div className="card">
          <div className="card-label">BotChain DEX Integration</div>
          {[
            { name:'BotChain DEX', desc:'Native DEX agent · @agentx_bot', icon:'⚡', status: tgStatus === 'success' ? 'CONNECTED' : 'SETUP REQUIRED' },
            { name:'BOT DEX Skills', desc:'Swap & Liquidity', icon:'◈', status: backendStatus === 'connected' ? 'CONNECTED' : 'OFFLINE' },
            { name:'BOT DEX Router', desc:'V3 Router', icon:'↗', status: backendStatus === 'connected' ? 'CONNECTED' : 'OFFLINE' },
          ].map((b, i) => (
        ...
        <div className="card">
          ...
          {[
            ...
            { name:'AgentXVault', addr: ADDRESSES.AgentXVault },
            ...
            <div key={i} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px', background:'rgba(255,255,255,0.02)', borderRadius:'8px', marginBottom:'8px' }}>
              ...
              <a href={'https://testnet-scan.botchain.ai/address/' + c.addr} target="_blank" rel="noreferrer"
...
export function Onboarding() {
  ...
  const suggestions = [
    ...
    'Generate passive income from my BOT',
  ];
  return (
