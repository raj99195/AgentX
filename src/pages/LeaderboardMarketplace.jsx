                  ...
                  {leaderboard.map((a, i) => (
                    <tr key={i}>
                      ...
                      <td style={{ fontFamily: 'JetBrains Mono', color: (a.net_pnl || 0) >= 0 ? '#4ade80' : '#ff4444', fontWeight: 600 }}>
                        {(a.net_pnl || 0) >= 0 ? '+' : ''}{(a.net_pnl || 0).toFixed(4)} BOT
                      </td>
...
export function Marketplace() {
  ...
  const handleStake = async (agentId) => {
    ...
    try {
      ...
      if (res.success) {
        setTxMsg(`✅ Staked ${amount} BOT on Agent #${agentId}`);
  ...
  const handleUnstake = async (agentId) => {
    ...
    try {
      ...
      if (res.success) {
        setTxMsg(`✅ Unstaked! Rewards: ${(res.data.rewards || 0).toFixed(4)} BOT`);
  ...
  return (
    <div className="page-wrap">
      <div style={{ marginBottom: '20px' }}>
        ...
        <p style={{ fontSize: '12px', color: '#555', marginTop: '4px' }}>Stake BOT on top agents — earn 20% of their rewards</p>
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '10px' }}>
            {myStakes.map((s, i) => (
              <div key={i} style={{ padding: '12px', background: 'rgba(255,255,255,0.02)', borderRadius: '8px', border: '0.5px solid rgba(255,255,255,0.08)' }}>
                ...
                <div style={{ fontSize: '16px', fontWeight: 600, color: '#fff', marginBottom: '4px' }}>{s.amount_mnt} BOT</div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', gap: '16px', fontSize: '11px', color: '#555' }}>
                      <span>Net P&L: <span style={{ color: (a.net_pnl || 0) >= 0 ? '#4ade80' : '#ff4444' }}>{(a.net_pnl || 0) >= 0 ? '+' : ''}{(a.net_pnl || 0).toFixed(4)} BOT</span></span>
                      <span>Win Rate: <span style={{ color: '#fff' }}>{a.winRate || 0}%</span></span>
                      <span>Earned: <span style={{ color: '#4ade80' }}>{(a.total_earned || 0).toFixed(4)} BOT</span></span>
                    </div>
                  </div>
                  ...
                  {myStake ? (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '4px' }}>
                      <div style={{ fontSize: '11px', color: '#4ade80' }}>Staked: {myStake.amount_mnt} BOT</div>
                    ...
                    <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                      ...
                      />
                      <span style={{ fontSize: '11px', color: '#444' }}>BOT</span>
