...
async function saveLoopState() {
  try {
    ...
    console.log('[AgentX] Supabase save skipped:', err.message?.slice(0, 60));
  }
}
  ...
  try {
    ...
    if (data) {
      ...
      if (data.running && !data.paused) {
        console.log('[AgentX] Resuming from previous state...');
        startLoop(data.current_goal);
      }
    }
  } catch (err) {
    console.log('[AgentX] No saved state found, starting fresh');
  }
}
...
function logActivity(agentName, action, status, txHash = null) {
  ...
  console.log(`[AgentX] ${agentName}: ${action} [${status}]`);
}

// ── Fetch agents from blockchain via Mantle RPC ───────────────
async function fetchAgentsOnChain() {
  try {
    const RPC = process.env.BOTCHAIN_TESTNET_RPC_URL || process.env.BOTCHAIN_RPC_URL || process.env.VITE_RPC_URL || 'https://rpc.bohr.life';
    const REGISTRY = process.env.VITE_AGENT_REGISTRY || '';
    ...
    console.log('[AgentX] getTotalAgents raw:', JSON.stringify(totalJson).slice(0, 150));
    const total = parseInt(totalJson.result || '0x0', 16);
    console.log('[AgentX] Total agents on-chain:', total);
    ...
    for (let i = 1; i <= Math.min(total, 20); i++) {
      try {
        ...
        console.log(`[AgentX] Agent #${i}: ${name} (${role}) active=${isActive} rep=${reputation}`);
      } catch (e) {
        console.log(`[AgentX] Agent #${i} decode error:`, e.message?.slice(0, 50));
      }
    }
    return agents;
  } catch (err) {
    console.log('[AgentX] fetchAgents failed:', err.message?.slice(0, 60));
    return [];
  }
}
...
async function fetchMarketData() {
  try {
    // Use BotChain price API
    const priceApi = process.env.BOTCHAIN_PRICE_API;
    if (priceApi) {
      const res = await fetch(priceApi, { signal: AbortSignal.timeout(5000) });
      const json = await res.json();
      const price = parseFloat(json?.price || json?.data?.price || json?.result?.price || 0);
      return { mntPrice: price, mntChange24h: 0 };
    }
...
// ── Fetch live BotChain DEX pool data ────────────────────────
async function fetchBotChainPools() {
  try {
    const factory = process.env.BOTCHAIN_DEX_FACTORY;
    const wbot = process.env.BOTCHAIN_WBOT;
    const usdt = process.env.BOTCHAIN_USDT;
    const rpc = process.env.BOTCHAIN_RPC_URL || process.env.BOTCHAIN_TESTNET_RPC_URL;

    if (!factory || !rpc) {
      console.log('[AgentX] BotChain DEX env vars missing');
      return null;
    }

    // Fetch BOT price from BotChain price API
    const priceRes = await fetch(process.env.BOTCHAIN_PRICE_API, { signal: AbortSignal.timeout(5000) });
    const priceData = await priceRes.json();
    const botPrice = parseFloat(priceData?.price || priceData?.data?.price || 0);

    // Return known pools from env-configured DEX
    return [
      {
        pair: 'WBOT/USDT',
        apr: '0.00',
    ...
    console.log('[AgentX] BotChain DEX fetch failed:', e.message?.slice(0, 80));
    return null;
  }
}

// Keep fetchByrealPools as alias for compatibility
const fetchByrealPools = fetchBotChainPools;

// ── Fetch DeFiLlama BotChain TVL ─────────────────────────────
async function fetchDeFiLlamaData() {
  try {
    const res = await fetch('https://api.llama.fi/v2/chains', { signal: AbortSignal.timeout(6000) });
    const chains = await res.json();
    const chainName = (process.env.VITE_CHAIN_NAME || 'bot chain').toLowerCase();
    const botchain = chains.find(c =>
      c.name?.toLowerCase() === chainName ||
      c.name?.toLowerCase() === 'botchain' ||
      c.name?.toLowerCase() === 'bot chain'
    );
    return { mantleTvl: botchain?.tvl || 0, mantleTvlChange: botchain?.change_1d || 0 };
  } catch {
  ...
  try {
    const response = await groq.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `You are AgentX AI, autonomous AI agent orchestrator on BotChain blockchain.
...
- pay: if health > 60 and active and totalDecisions > 0, set amount 0.001-0.005 BOT based on performance
...
[{ "agentId": number, "action": string, "amount": number, "reason": string, "bestPool": string|null, "riskAction": string|null, "strategyNote": string|null }]`
        },
        {
          ...
          content: JSON.stringify({
            ...
            goal: (goal || 'maximize yield on BotChain').slice(0, 80),
          })
        }
      ],
      temperature: 0.2,
    ...
    if (start === -1 || end === -1) {
      console.log('[AgentX] Groq V2 no JSON array found, raw:', text.slice(0, 100));
      return [];
    }
    ...
    console.log('[AgentX] Groq V2 failed:', err.message?.slice(0, 80));
...
[{ "agentId": number, "action": "keep"|"pay"|"fire"|"hire_new", "amount": number_in_BOT, "reason": "short reason" }]
No markdown, no explanation.`
        },
        {
          ...
          content: JSON.stringify({
            ...
            market: marketData,
            goal: goal || 'maximize yield on BotChain DeFi',
          })
        }
      ],
      temperature: 0.2,
    ...
    console.log('[AgentX] Groq decision failed:', err.message?.slice(0, 60));
    return [];
  }
}
...
function detectAnomalies(market, pools, prevMarket) {
  ...
  if (prevMarket && Math.abs(market.mntChange24h - prevMarket.mntChange24h) > 5) {
    anomalies.push({
      type: 'PRICE_SPIKE',
      severity: 'HIGH',
      msg: `${process.env.VITE_NATIVE_TOKEN || 'BOT'} price change shifted ${(market.mntChange24h - prevMarket.mntChange24h).toFixed(2)}% since last cycle`,
    });
  }
  ...
  if (market.mntChange24h < -5) {
    anomalies.push({
      type: 'MARKET_CRASH',
      severity: 'HIGH',
      msg: `${process.env.VITE_NATIVE_TOKEN || 'BOT'} down ${Math.abs(market.mntChange24h).toFixed(2)}% today — defensive strategy recommended`,
    });
  }
  ...
  try {
    const response = await groq.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are AgentX AI. Write a 2-sentence plain English summary of what the autonomous agents decided this cycle and why. Be specific and confident. No markdown.',
        },
        {
          role: 'user',
          content: JSON.stringify({
...
async function runLoopTick() {
  if (loopState.paused || loopState.emergencyStop) {
    console.log('[AgentX] Skipping tick — paused or emergency stop');
    return;
  }
  ...
  if (chatIds.length > 0 && decisions.length > 0) {
    ...
    const msg = `🤖 <b>AgentX Cycle #${loopState.cycleCount}</b>\n\n📊 Agents: ${activeAgents} active\n💹 BOT: $${market.mntPrice.toFixed(4)} (${market.mntChange24h.toFixed(2)}%)${poolLine}\n🛡️ Risk: ${risk.level} (${risk.score}/10)\n\n<b>AI Decisions:</b>\n${summary}\n\n<i>Autonomous AI Agent Economy on BotChain</i>`;
...
function startLoop(goal = null) {
  ...
  saveLoopState();
  broadcast('loop_state', loopState);
  console.log(`[AgentX] Started — every ${loopState.intervalMs / 60000} min`);
}
...
httpServer.listen(PORT, () => {
  ...
  console.log(`BotChain DEX + Telegram + Supabase connected!`);
